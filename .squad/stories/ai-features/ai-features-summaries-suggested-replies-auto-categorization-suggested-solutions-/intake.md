...
```
AI Features -- Story 08 of the v2 full-12-area build
```
...
- specs/v2/01-requirements.md  -- EARS requirements; THIS story delivers: REQ-AI-1..7
...
Add the AiProvider port with ClaudeProvider (@anthropic-ai/sdk, model + key from env)
and a deterministic MockAiProvider (tests use the mock). Features: ticket summary
(cached AiInteraction), suggested reply (insert/edit/send, never auto), auto-
categorization on create behind a config flag with a confidence threshold +
AI_CATEGORIZED event, suggested solutions (FTS retrieval over KB + optional AI re-rank),
and an AI chatbot for portal/live-chat that can escalate to a queued ChatSession. Log
every call (AiInteraction), rate-limit per user, degrade gracefully on provider error
(no 500).
```
...
```
Every requirement in specs/v2/01-requirements.md under this story's area (REQ-AI-1..7),
