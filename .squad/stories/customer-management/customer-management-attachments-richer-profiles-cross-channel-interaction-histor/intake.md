...
```
Customer Management -- Story 03 of the v2 full-12-area build
```
...
- specs/v2/01-requirements.md  -- EARS requirements; THIS story delivers: REQ-CUSX-1..5
...
Add Attachment infra (disk storage under a config root + DB rows, size/type limits),
richer Customer fields (tags, vipLevel, preferredChannel, locale, metadata), the
Conversation/Message model (substrate for channels + portal), the unified
interaction-history endpoint, and customer merge.
```
...
```
Every requirement in specs/v2/01-requirements.md under this story's area (REQ-CUSX-1..5),
