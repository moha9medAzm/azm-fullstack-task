...
```
SLA & Automation -- Story 06 of the v2 full-12-area build
```
...
- specs/v2/01-requirements.md  -- EARS requirements; THIS story delivers: REQ-AUTO-1..7
...
Add AutomationRule + a PURE rule engine (triggers: TICKET_CREATED/UPDATED/
SLA_*_BREACHED/SCHEDULED; conditions on priority/category/channel/VIP/branch/department/
tag/unassigned/business-hours; actions: assign(round-robin|least-loaded|specific)/set
fields/tag/escalate/notify/canned-reply). Automatic assignment strategies (pure,
tested). Escalation rules replace the hard-coded sweep (seed current behaviour as the
default rule). Notification entity + Notifier (in-app real + EmailNotifier adapter) +
bell menu. RuleRun for idempotency (one fire per ticket/rule/trigger).
```
...
```
Every requirement in specs/v2/01-requirements.md under this story's area (REQ-AUTO-1..7),
