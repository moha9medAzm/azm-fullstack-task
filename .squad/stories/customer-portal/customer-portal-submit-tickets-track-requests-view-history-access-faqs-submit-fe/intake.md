...
```
Customer Portal -- Story 09 of the v2 full-12-area build
```
...
- specs/v2/01-requirements.md  -- EARS requirements; THIS story delivers: REQ-PORT-1..6
...
Add CustomerUser + portal auth (JWT role=CUSTOMER scoped to one customer; cannot call
agent/admin routes). A separate /portal/* route tree + layout with its own nav, branch
branding, and full i18n/RTL. Submit ticket (channel PORTAL, runs automation rules),
list/open ONLY own tickets showing status + PUBLIC comments + own replies (never
internal notes), post replies (inbound Message + COMMENTED), browse/search the public
KB, and CSAT (1-5 + comment, tokenised, one per ticket) on resolution.
```
...
```
Every requirement in specs/v2/01-requirements.md under this story's area (REQ-PORT-1..6),
