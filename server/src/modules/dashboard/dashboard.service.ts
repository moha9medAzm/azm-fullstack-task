...
export async function getDashboardStats(actorId: string) {
  ...
  const [statusGroups, priorityGroups, myOpen, unassigned, breachingResolution, breachingResponse, resolvedLast7d] =
    await Promise.all([
  ...
  const byPriority = Object.fromEntries(TICKET_PRIORITIES.map((p) => [p, 0])) as Record<string, number>;
