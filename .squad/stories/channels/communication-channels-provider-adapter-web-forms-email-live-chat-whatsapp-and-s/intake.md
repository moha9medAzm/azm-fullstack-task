...
```
Communication Channels -- Story 05 of the v2 full-12-area build
```
...
- specs/v2/01-requirements.md  -- EARS requirements; THIS story delivers: REQ-CHAN-1..7
...
Add the ChannelProvider port + registry. Web Forms: public rate-limited endpoint ->
Conversation + optional auto-ticket. Live Chat: ChatSession queue, claim, polling
messages, convert-to-ticket. Email: nodemailer out when SMTP_URL set else mock; inbound
webhook parser threading by subject ref / In-Reply-To. WhatsApp/SMS: TwilioProvider
skeleton (env-guarded) + MockSmsProvider + inbound webhook. Agent reply picks the
outbound channel. All inbound webhooks signature-verified (401 if unsigned).
```
...
```
Every requirement in specs/v2/01-requirements.md under this story's area (REQ-CHAN-1..7),
