...
```
Knowledge Base -- Story 07 of the v2 full-12-area build
```
...
- specs/v2/01-requirements.md  -- EARS requirements; THIS story delivers: REQ-KB-1..5
...
Add KbArticle (FAQ/GUIDE/SOLUTION, DRAFT/PUBLISHED/ARCHIVED, INTERNAL/PUBLIC, slug
unique per branch scope, versioned via KbArticleVersion), SQLite FTS5 full-text search
with ranked <mark> snippets (document the Postgres tsvector path), article<->ticket
linking (KB_LINKED event + usefulCount), and public read endpoints for the portal.
```
...
```
Every requirement in specs/v2/01-requirements.md under this story's area (REQ-KB-1..5),
