...
```
Ticket Management -- Story 04 of the v2 full-12-area build
```
...
- specs/v2/01-requirements.md  -- EARS requirements; THIS story delivers: REQ-TKTX-1..6
...
Add Tag + TicketTag (filterable), TicketWatcher (follow/unfollow), ticket merge (move
children, MERGED event, lock B), ticket split (new ticket from selected comments,
SPLIT_FROM link), and bulk actions (assign/priority/status/tag/close) that still honour
the transition guard + RBAC + audit.
```
...
```
Every requirement in specs/v2/01-requirements.md under this story's area (REQ-TKTX-1..6),
