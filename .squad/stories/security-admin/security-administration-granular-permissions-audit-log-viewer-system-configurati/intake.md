...
```
Security & Administration -- Story 02 of the v2 full-12-area build
```
...
- specs/v2/01-requirements.md  -- EARS requirements; THIS story delivers: REQ-PERM-1..7
...
Add the permissions layer (permission strings + per-user grants + role defaults incl.
SUPERVISOR), the audit-log viewer (TicketEvent + new AdminEvent, filters + CSV), and the
system-configuration screen. Replace authorize(role) with requirePermission('x') across
existing routes via a role->permissions default map so nothing regresses.
```
...
```
Every requirement in specs/v2/01-requirements.md under this story's area (REQ-PERM-1..7),
